...
class LocalDahoasRmstaticDataset(PromptRawDataset):
    def __init__(self, output_path, seed, local_rank):
        super().__init__(output_path, seed, local_rank)
        self.dataset_name = "Dahoas/rm-static"
        self.dataset_name_clean = "Dahoas_rm_static"
        self.raw_datasets = load_dataset('parquet', data_dir='/home/WangXu/huggingface_data/Dahoas/rm-static')
...
class DahoasFullhhrlhfDataset(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank):
        ...
        self.raw_datasets = load_dataset('parquet', data_dir='/home/WangXu/huggingface_data/Dahoas_full-hh-rlhf')
...
class DahoasSyntheticinstructgptjpairwiseDataset(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank):
        ...
        self.raw_datasets = load_dataset(
            'parquet', data_dir='/home/WangXu/huggingface_data/Dahoas_synthetic-instruct-gptj-pairwise')
...
class YitingxieRlhfrewarddatasetsDataset(PromptRawDataset):
    ...
    def __init__(self, output_path, seed, local_rank):
        ...
        self.raw_datasets = load_dataset('parquet', data_dir='/home/WangXu/huggingface_data/yitingxie_rlhf-reward-datasets')
...
# Chinese dataset
class LocalWangrui6ZhihuKOLDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank):
        super().__init__(output_path, seed, local_rank)
        self.dataset_name = "wangrui6/Zhihu-KOL"
        self.dataset_name_clean = "wangrui6_Zhihu_KOL"
        self.raw_datasets = load_dataset("parquet", data_dir="/home/WangXu/huggingface_data/wangrui6/Zhihu-KOL")
...
# Chinese dataset
class LocalCohereMiraclzhqueries2212Dataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank):
        super().__init__(output_path, seed, local_rank)
        self.dataset_name = "Cohere/miracl-zh-queries-22-12"
        self.dataset_name_clean = "Cohere_miracl_zh_queries_22_12"
        self.raw_datasets = load_dataset("parquet", data_dir="/home/WangXu/huggingface_data/Cohere/miracl-zh-queries-22-12")
...
# Chinese local dataset
class LocalBelle2MDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank):
        super().__init__(output_path, seed, local_rank)
        self.dataset_name = "belle2M"
        self.dataset_name_clean = "belle2M"
        self.raw_datasets = load_from_disk("/home/WangXu/huggingface_data/belle2M")

    def get_train_data(self):
        from .data_utils import get_raw_dataset_split_index
        dataset = self.raw_datasets["train"].select(range(10000))
        index = get_raw_dataset_split_index(self.local_rank, self.output_path,
                                            self.dataset_name_clean,
                                            self.seed, "train_eval", "9,1", 0,
                                            len(dataset))
    ...
    def get_chosen(self, sample):
        if sample['output'][0] is not None:
    ...
    def get_prompt_and_chosen(self, sample):
        if sample['instruction'] is not None and sample['output'][
                0] is not None:
            return " Human: " + sample['instruction'] + " Assistant: " + sample['output'][0]
        return None

    def get_prompt_and_rejected(self, sample):
        if sample['instruction'] is not None and sample['output'][0] is not None:
            return " Human: " + sample['instruction'] + " Assistant: " + sample['output'][0]
        return None


# Chinese local dataset
class LocalHelloSimpleAIHC3ChineseDataset(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank):
        super().__init__(output_path, seed, local_rank)
        self.dataset_name = "Hello-SimpleAI/HC3-Chinese"
        self.dataset_name_clean = "Hello_SimpleAI_HC3_Chinese"
        # self.raw_datasets = load_dataset("jsonl", data_dir="/home/WangXu/huggingface_data/Hello-Simple/miracl-zh-queries-22-12")
        # self.raw_datasets = load_dataset("Hello-SimpleAI/HC3-Chinese", "all")
        self.raw_datasets = load_from_disk("/home/WangXu/huggingface_data/Hello-Simple/miracl-zh-queries-22-12")
        # self.raw_datasets.save_to_disk(dataset_dict_path='/home/WangXu/a')
    ...
    def get_rejected(self, sample):
        if sample['chatgpt_answers'][0] is not None:
            return " " + sample['chatgpt_answers'][0]
        # print(
        #     f"Warning: dataset {self.dataset_name} does not include rejected response."
        # )
        return None
    ...
    def get_prompt_and_rejected(self, sample):
        if sample['question'] is not None and sample['human_answers'][0] is not None:
            return " Human: " + sample['question'] + " Assistant: " + sample['chatgpt_answers'][0]
        return None
