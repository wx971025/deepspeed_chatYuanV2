...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    if dataset_name == "Dahoas-rm-static-local":    # Custom local english dataset
        return raw_datasets.LocalDahoasRmstaticDataset(output_path, seed,
                                                       local_rank)
    elif dataset_name == "wangrui6-Zhihu-KOL-local":    # Custom local chinese dataset
        return raw_datasets.LocalWangrui6ZhihuKOLDataset(output_path, seed,
                                                         local_rank)
    elif dataset_name == "Dahoas/rm-static":
        return raw_datasets.DahoasRmstaticDataset(output_path, seed,
                                                  local_rank)
    elif dataset_name == "Dahoas_full-hh-rlhf":
        return raw_datasets.DahoasFullhhrlhfDataset(output_path, seed,
                                                    local_rank)
    elif dataset_name == "Dahoas_synthetic-instruct-gptj-pairwise":
        return raw_datasets.DahoasSyntheticinstructgptjpairwiseDataset(
            output_path, seed, local_rank)
    elif dataset_name == "yitingxie_rlhf-reward-datasets":
        ...
        return raw_datasets.Wangrui6ZhihuKOLDataset(output_path, seed,
                                                    local_rank)
    elif dataset_name == "Cohere-miracl-zh-queries-22-12":      # Custom chinese only sft dataset
        return raw_datasets.LocalCohereMiraclzhqueries2212Dataset(
            output_path, seed, local_rank)
        ...
        return raw_datasets.LocalHelloSimpleAIHC3ChineseDataset(
            output_path, seed, local_rank)
    
    elif dataset_name == "belle2M":
        return raw_datasets.LocalBelle2MDataset(output_path, seed, local_rank)
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    reject_dataset = []
    if train_phase == 1:
        print_rank_0(f'current_dataset len is {len(current_dataset)}', torch.distributed.get_rank())
