...
def parse_args():
    ...
    parser.add_argument("--num_padding_at_beginning",
                        type=int,
                        default=1,
                        help=
...
def main():
    ...
    print_rank_0(rm_model, args.global_rank)

    if args.lora_dim > 0:
        print_rank_0(f'****** lora has been opened! ******')
    ...
    def evaluation_reward(model, eval_dataloader):
        print(f'eval_len is {len(eval_dataloader)}')
        model.eval()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        rm_model.train()
        mean_loss = 0
        print(f'train_len is {len(train_dataloader)}')
